...
// Given an array of integers input and an integer k, find the maximum sum of any contiguous subarray of exactly k
// elements.
int max_sum_subarray_size_k(const std::vector<int>& input, int k)
{
    ...
    int current_sum = std::accumulate(input.begin(), input.begin() + k, 0);
