...
void MergeSort::merge_(std::vector<int64_t> &array, std::vector<int64_t> &help, size_t start, size_t middle, size_t end)
{
	...
	for (size_t index = start; index <= end; ++index) {
		if (first_half_index > middle) { array[index] = help[second_half_index++]; }
		else if (second_half_index > end) { array[index] = help[first_half_index++]; }
		else if (help[second_half_index] < help[first_half_index]) { array[index] = help[second_half_index++]; }
		else { array[index] = help[first_half_index++]; }
	}
