...
{
public:
    explicit SetupSortingTests() : random_engine_{std::random_device{}()} {}
    ...
    static bool compare_pointers(const int* a, const int* b) { return *a < *b; }
    ...
    std::vector<int*> get_N_random_number_vector_of_pointers(const int lower_bound, const int upper_bound,
                                                             const size_t size)
    {
        int_distribution_ = std::uniform_int_distribution{lower_bound, upper_bound};
        std::vector<int*> result;
