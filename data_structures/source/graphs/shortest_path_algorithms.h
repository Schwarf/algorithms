...
std::vector<DistanceType> distances_from_source_dijkstra(VertexType source,
    ...
    while (!pq.empty()) {
        auto [distance, current_vertex] = pq.top();
        pq.pop();
        ...
        for (const auto &edge: graph[current_vertex]) {
            auto [next_vertex, distance_current_to_next] = edge;
            auto distance_start_to_next = distance + distance_current_to_next;

            if (distance_start_to_next < distances_from_source_vertex[next_vertex]) {
                distances_from_source_vertex[next_vertex] = distance_start_to_next;
                pq.emplace(distance_start_to_next, next_vertex);
            }
        }
    }
