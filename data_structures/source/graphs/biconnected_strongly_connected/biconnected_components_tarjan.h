...
// given as std::vector<std::vector<'integral_type'>> where each inner vector consists of two nodes [source,
// destination]
//
...
void dfs_bcc_tarjan(NodeType current_node, NodeType parent, const UndirectedGraph<NodeType>& graph,
                    ...
                    std::set<std::set<NodeType>>& biconnected_components, int& discovery_index)
{
    ...
    for (auto neighbor : graph.get_neighbors(current_node))
    {
        if (!discovery_order.contains(neighbor)) // If neighbor is not visited
        {
            ...
            dfs_bcc_tarjan(neighbor, current_node, graph, discovery_order, ancestor_reachability_value, edge_stack,
            ...
            if ((parent == -1 && children > 1) ||
                (parent != -1 && ancestor_reachability_value[neighbor] >= discovery_order[current_node]))
            {
