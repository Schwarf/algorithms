...
template <typename T>
void dfs_tarjan_undirected(std::vector<std::vector<T>>& graph, T node, std::vector<bool>& visited, std::vector<T>& discovery_time,
                std::vector<T>& ancestor_reachability_value, std::vector<T>& parent, int & time)
{
    ...
    for (auto neighbor_node : graph[node])
    {
        ...
        else if (neighbor_node != parent[node])
        {
            ...
            ancestor_reachability_value[node] = std::min(ancestor_reachability_value[node], discovery_time[neighbor_node]);
...
template <typename T>
std::pair<std::vector<T>, std::vector<T>> tarjan_algorithm_undirected(std::vector<std::vector<T>>& edges, int number_of_nodes)
{
