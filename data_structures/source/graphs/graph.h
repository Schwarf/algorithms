...
template <>
class UndirectedGraph<uint64_t>
{
public:
    UndirectedGraph(size_t num_nodes, const std::initializer_list<std::pair<uint64_t, uint64_t>>& edges)
        : adjacency_list(num_nodes), node_count(num_nodes), edge_count(0)
    {
        for (const auto& edge : edges)
        {
            add_edge(edge.first, edge.second);
    ...
    // Add an undirected edge between 'source_node' and 'destination_node'
    void add_edge(const uint64_t source_node, const uint64_t destination_node)
    {
        ...
        if (source_node != destination_node &&
            std::find(adjacency_list[source_node].begin(), adjacency_list[source_node].end(), destination_node) == adjacency_list[source_node].end())
        {
    ...
    {
        std::vector<uint64_t> nodes(node_count);
    ...
    // Get adjacent nodes of a given node
    const std::vector<uint64_t>& get_neighbors(uint64_t node) const
    {
        validate_node(node);
...
private:
    std::vector<std::vector<uint64_t>> adjacency_list; // Adjacency list for each node
    ...
    void validate_node(const uint64_t node) const
    {
        if (node >= node_count)
        {
