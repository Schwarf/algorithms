...
TEST(flatten_binary_search_tree_to_linked_list, flattens_tree_in_inorder)
{
    auto root = new TreeNode<int>(25);
    root->left = new TreeNode<int>(15);
    root->right = new TreeNode<int>(50);
    ...
    auto head = flatten_to_linked_list_inorder(root);

    const std::vector<int> expected_forward{10, 15, 22, 25, 35, 50, 70};
    const std::vector<int> expected_backward{70, 50, 35, 25, 22, 15, 10};
    ...
    EXPECT_EQ(nullptr, tail_of(head)->right);

    delete_flattened_list(head);
}

TEST(flatten_binary_search_tree_to_linked_list, inorder_preserves_existing_nodes)
{
    ...
    EXPECT_EQ(left, left_left->right);
    EXPECT_EQ(left_left, left->left);
    EXPECT_EQ(left_right, left->right);
    EXPECT_EQ(left, left_right->left);
    EXPECT_EQ(root, left_right->right);
    EXPECT_EQ(left_right, root->left);
    EXPECT_EQ(right_left, root->right);
    EXPECT_EQ(root, right_left->left);
    EXPECT_EQ(right, right_left->right);
    EXPECT_EQ(right_left, right->left);
    EXPECT_EQ(right_right, right->right);
    EXPECT_EQ(right, right_right->left);
...
TEST(flatten_binary_search_tree_to_linked_list, inorder_handles_single_node)
{
    auto root = new TreeNode<int>(42);
...
TEST(flatten_binary_search_tree_to_linked_list, inorder_handles_empty_tree)
{
    TreeNode<int>* root = nullptr;
