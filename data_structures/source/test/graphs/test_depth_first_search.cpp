...
protected:
	...
	Graph<int, int> get_graph_without_cycle(int start_node_id, GraphNodePtr<int, int> &start_node) const
	{
		const std::shared_ptr<GraphNode<int, int>> node1(new GraphNode<int, int>(id1, data1));
		const std::shared_ptr<GraphNode<int, int>> node2(new GraphNode<int, int>(id2, data2));
		const std::shared_ptr<GraphNode<int, int>> node3(new GraphNode<int, int>(id3, data3));
...
TEST_F(SetupDFSGraph, test_depth_first_search_has_cycle_node1)
{
	GraphNodePtr<int, int> start_node = nullptr;
	int start_node_id = 1;
	auto graph = get_graph(start_node_id, start_node);
	EXPECT_FALSE(graph.has_cycle());
	get_depth_first_search_tree<int, int>(graph, start_node);
	EXPECT_TRUE(graph.has_cycle());
}


TEST_F(SetupDFSGraph, test_depth_first_search_has_cycle_node6)
{
	GraphNodePtr<int, int> start_node = nullptr;
	int start_node_id = 6;
	auto graph = get_graph(start_node_id, start_node);
...
TEST_F(SetupDFSGraph, test_depth_first_search_has_no_cycle_node1)
{
	GraphNodePtr<int, int> start_node = nullptr;
	int start_node_id = 1;
	auto graph = get_graph_without_cycle(start_node_id, start_node);
	EXPECT_FALSE(graph.has_cycle());
	get_depth_first_search_tree<int, int>(graph, start_node);
	EXPECT_FALSE(graph.has_cycle());
}
...
TEST_F(SetupDFSGraph, test_depth_first_search_has_no_cycle_node4)
{
	GraphNodePtr<int, int> start_node = nullptr;
	int start_node_id = 4;
	auto graph = get_graph_without_cycle(start_node_id, start_node);
...
TEST_F(SetupDFSGraph, test_depth_first_search_has_no_cycle_node2)
{
	GraphNodePtr<int, int> start_node = nullptr;
	int start_node_id = 2;
	auto graph = get_graph_without_cycle(start_node_id, start_node);
