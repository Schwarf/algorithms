...
TEST(FindShortestEdgeInSubgraph, ZeroCase_BFS)
{
    ...
    auto result = minimum_number_of_edges_to_create_connected_graph(number_of_required_vertices, input);
...
TEST(FindShortestEdgeInSubgraph, ZeroCase_DFS)
{
    ...
    int expected_result{0};
    unsigned short number_of_required_vertices{6};
    auto result = minimum_number_of_edges_to_create_connected_graph(number_of_required_vertices, input, false);
    EXPECT_EQ(expected_result, result);
}
