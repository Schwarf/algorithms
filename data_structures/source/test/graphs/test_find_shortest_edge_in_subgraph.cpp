...
{
    std::vector<std::vector<unsigned int>> input{{1, 2, 3}, {2, 4, 5}, {1, 3, 7}, {3, 4, 5}, {1, 5, 1}};
...
{
    std::vector<std::vector<unsigned int>> input{{1, 2, 3}, {2, 4, 5}, {1, 3, 7}, {3, 4, 5},
                                                 {1, 5, 1}, {6, 7, 2}, {6, 8, 2}, {7, 8, 2}};
    unsigned int start_vertex{1};
...
TEST(FindShortestEdgeInSubgraph, DFSrecursiveallconnected1)
{
    ...
    unsigned int not_connected_value{10000};
    unsigned int expected_result{1};
    auto result = find_smallest_edge_in_sub_graph_dfs_recursive(input, start_vertex, end_vertex, max_edge_value,
                                                                not_connected_value);
    EXPECT_EQ(expected_result, result);
}
