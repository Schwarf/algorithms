...
TEST(TestLFUCache, test_value_update_increases_frequency)
{
    ...
    cache.put(1, 100);
    cache.get(1);
    // Update the value of key '1' which should also count as an access and increase the frequency
    cache.put(1, 200);

    cache.put(2, 300);
    cache.get(2);

    // Now, insert a third item which should trigger eviction of key '2'
    cache.put(3, 400);
