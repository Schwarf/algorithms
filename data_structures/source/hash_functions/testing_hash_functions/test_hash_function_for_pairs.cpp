...
TEST(TestUniformityOfPairHashes, integer_integer_pairs_commutative)
{
    ...
    auto test =
        UniformityOfPairHashFunction<decltype(commutative_pair_hash<int, int>()), int, int>::compute_chi_square_value(
            commutative_pair_hash<int, int>(), input, number_of_buckets);
...
TEST(TestUniformityOfPairHashes, integer_integer_pairs_non_commutative)
{
    ...
    auto test = UniformityOfPairHashFunction<decltype(non_commutative_pair_hash<int, int>()), int,
                                             int>::compute_chi_square_value(non_commutative_pair_hash<int, int>(),
                                                                            input, number_of_buckets);
    constexpr double chi_square{1074.3};
    std::cout << test << std::endl;
    EXPECT_TRUE(test < chi_square);
}
...
TEST(TestUniformityOfPairHashes, integer_double_pairs_commutative)
{
    ...
    auto test = UniformityOfPairHashFunction<decltype(commutative_pair_hash<int, double>()), int,
                                             double>::compute_chi_square_value(commutative_pair_hash<int, double>(),
                                                                               input, number_of_buckets);
    constexpr double chi_square{1074.3};
    std::cout << test << std::endl;
    EXPECT_TRUE(test < chi_square);
...
TEST(TestUniformityOfPairHashes, integer_double_pairs_non_commutative)
{
    ...
    auto test = UniformityOfPairHashFunction<decltype(non_commutative_pair_hash<int, double>()), int,
                                             double>::compute_chi_square_value(non_commutative_pair_hash<int, double>(),
                                                                               input, number_of_buckets);
    constexpr double chi_square{1074.3};
    std::cout << test << std::endl;
    EXPECT_TRUE(test < chi_square);
