...
public:
    ...
    void test(IHashFunction& hash_function)
    {
        ...
        double equal_distribution_expectation =
            static_cast<double>(words_.size()) / static_cast<double>(hash_table_size);
        size_t hash_value{};
        ...
        for (const auto& hash_sum_element : hashes_)
        {
            sum += (equal_distribution_expectation - hash_sum_element) *
                (equal_distribution_expectation - hash_sum_element) / equal_distribution_expectation;
        }
