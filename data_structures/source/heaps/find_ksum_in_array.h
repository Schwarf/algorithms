...
// You are given an integer array nums and a positive integer k. You can choose any subsequence of the array and sum all
// of its elements together. We define the K-Sum of the array as the kth largest subsequence sum that can be obtained
// (not necessarily distinct). Return the K-Sum of the array. A subsequence is an array that can be derived from another
// array by deleting some or no elements without changing the order of the remaining elements. Note that the empty
// subsequence is considered to have a sum of 0.
#include <algorithm>
#include <iostream>
#include <queue>
#include <vector>
...
long long find_ksum_in_array(std::vector<int>& input, int k)
{
    ...
    std::priority_queue<std::pair<long long, input_index>, std::vector<std::pair<long long, input_index>>,
                        std::greater<std::pair<long long, input_index>>>
        pq;
