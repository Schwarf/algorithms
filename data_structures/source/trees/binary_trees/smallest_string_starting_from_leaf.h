...
void
smallest_string_starting_from_leaf(TreeNode<int> *root, std::string &smallest_string, std::string current_string = "") {
    if (!root)
        return;
    current_string = static_cast<char>(root->value + 'a') + current_string;
    if (!root->left && !root->right) {
        if (smallest_string == "" || smallest_string > current_string)
