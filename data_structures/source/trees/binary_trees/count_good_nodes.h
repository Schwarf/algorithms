...
// value greater than X. Return the number of good nodes in the binary tree. Each node's value is between [-10^5, 10^5].
...
int count_good_nodes_dfs_recursive(TreeNode<T>* root, T max = std::numeric_limits<T>::min())
{
    if (!root)
        return 0;
    if (root->value >= max)
        return 1 + count_good_nodes_dfs_recursive(root->left, root->value) +
            count_good_nodes_dfs_recursive(root->right, root->value);
