...
// Given the root of a binary tree, replace the valueue of each node in the tree with the sum of all its cousins'
// valueues. Two nodes of a binary tree are cousins if they have the same depth with different parents. Return the root
...
TreeNode<T>* replace_values_with_cousin_sum_running_sum(TreeNode<T>* root)
{
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            T sibling_sum = (current->left != nullptr ? current->left->value : 0) +
                (current->right != nullptr ? current->right->value : 0);
