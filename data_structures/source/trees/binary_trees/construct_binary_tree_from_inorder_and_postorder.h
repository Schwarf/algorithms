...
template <typename T>
TreeNode<T>* construct_tree_post_in(std::vector<T>& postorder, int left_in_order_index, int right_in_order_index,
                                    int& post_order_index, std::unordered_map<T, int>& inorder_value_to_index)
{
    ...
    root->right =
        construct_tree_post_in(postorder, pivot + 1, right_in_order_index, post_order_index, inorder_value_to_index);
    root->left =
        construct_tree_post_in(postorder, left_in_order_index, pivot - 1, post_order_index, inorder_value_to_index);
    return root;
}
