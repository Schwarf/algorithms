...
template <typename T>
std::string serialize_subtrees(TreeNode<T>* node, std::unordered_map<std::string, int>& subtrees,
                               std::vector<TreeNode<T>*>& duplicates)
{
    ...
    std::string subtree_string =
        left + "," + right + "," + std::to_string(node->value); // Serialize the current subtree
