...
int longest_string_chain(std::vector<std::string>& words)
{
    ...
    std::sort(words.begin(), words.end(),
              [](const std::string& word1, const std::string& word2) { return word1.length() < word2.length(); });
    ...
    for (const auto& current_word : words)
    {
        ...
        {
            const std::string possible_predecessor = current_word.substr(0, i) + current_word.substr(i + 1);
