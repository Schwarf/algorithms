...
long long get_maximum_cost_dp_brute_force(std::vector<std::vector<int>>& matrix)
{
    ...
    for (int row{1}; row < rows; ++row)
    {
        for (int col{}; col < columns; ++col)
        {
            ...
            {
                dp[row][col] =
                    std::max(dp[row][col], dp[row - 1][col_before] + matrix[row][col] - std::abs(col - col_before));
            }
        }
    }
...
long long get_maximum_cost_dp(std::vector<std::vector<int>>& matrix)
{
    ...
    for (int row{}; row < rows - 1; ++row)
    {
        ...
        // it is chosen. If the any value left of the current index is the largest (including the subtraction) we find
        // it.
