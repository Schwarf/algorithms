...
int dp_helper(std::vector<std::vector<int>>& books, int shelf_width, std::vector<std::vector<int>>& memo,
              int book_index, int remaining_width, int max_height)
{
    ...
    auto put_on_new_shelf_height = max_height +
        dp_helper(books, shelf_width, memo, book_index + 1, shelf_width - current_book[0], current_book[1]);
    if (remaining_width >= current_book[0])
    {
        int remain_on_shelf_height =
            dp_helper(books, shelf_width, memo, book_index + 1, remaining_width - current_book[0], new_height);
