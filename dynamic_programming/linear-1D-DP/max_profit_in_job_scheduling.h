...
int job_scheduling(std::vector<int>& start_time, std::vector<int>& end_time, std::vector<int>& profit)
{
    ...
    std::sort(jobs.begin(), jobs.end(), [](const Job& j1, const Job& j2) { return j1.start < j2.start; });
...
int job_scheduling_top_down(const std::vector<int>& start, const std::vector<int>& end, std::vector<int>& profit)
{
    struct Job
    {
        int start;
    ...
    std::function<int(int)> dfs = [&](int current_job_index) -> int
    {
        ...
        int next_job_index =
            int(std::lower_bound(sorted_starts.begin(), sorted_starts.end(), jobs[current_job_index].end) -
                sorted_starts.begin());
...
int job_scheduling_compact_bottom_up(const std::vector<int>& start, const std::vector<int>& end,
                                     std::vector<int>& profit)
{
    ...
    auto pred = [&](int i) -> int
    { return int(std::upper_bound(sorted_ends.begin(), sorted_ends.end(), jobs[i].start) - sorted_ends.begin()) - 1; };
