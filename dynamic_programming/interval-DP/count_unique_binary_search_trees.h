...
int count_unique_BST_recursive(int n)
{
    ...
    for (int i{1}; i <= n; ++i)
    {
        ...
        // left tree has i-1 values (1.. i-1) and the right tree has n-i values (i+1..n). Now since each value
        // between 1..n can be the root value we multiply the number of left-values and right-values for each i and add
        // them up.
