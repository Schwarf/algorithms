...
int min_cost_to_reach_end(std::vector<int> &cost)
{
	int previous{};
	int before_previous{};
	if (cost.size() == 2)
	...
	for (int i = 2; i <= cost.size(); ++i) {
		int one_step = previous +  cost[i - 1] ;
		int two_step = before_previous + cost[i - 2];
...
int main()
{
	...
	auto test3 =
		std::pair<std::vector<int>, int>{std::vector<int>{1, 12, 419, 181, 17, 19, 0, 71, 26, 1, 2, 3, 4, 5, 6, 7}, 248};
