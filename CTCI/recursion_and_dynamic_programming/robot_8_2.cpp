...
bool getPath_memo(std::vector<std::vector<bool>> & maze, int row, int col, std::vector<std::pair<int, int>> & points, std::unordered_set<std::pair<int, int>, pair_hash> & memo)
{
	if(row < 0 || col < 0 || !maze[row][col])
		return false;
	...
	bool origin_reached = (row == 0) && (col == 0);
	if(origin_reached || getPath_memo(maze, row-1, col, points, memo) || getPath_memo(maze, row, col-1, points, memo))
	{
		points.push_back(std::make_pair(row, col));
		return true;
	}
