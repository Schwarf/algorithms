...
template <typename T, typename ObjectFunctionType, typename PerturbationFunctionType,
          template <typename...> class ContainerType, template <typename...> class InnerContainerType>
    ...
    std::invocable<PerturbationFunctionType, ContainerType<InnerContainerType<T>>&,
                   decltype(std::declval<std::mt19937>())&> &&
    ReturnTypeIsDouble<ObjectFunctionType, T, InnerContainerType>
double FunctionA(ContainerType<InnerContainerType<T>>& container, ObjectFunctionType func,
...
template <typename T, template <typename...> class ContainerType, template <typename...> class InnerContainerType>
...
template <typename T, template <typename> typename CoordinateType, template <typename> typename InnerContainerType,
typename FunctionType> double FunctionA(const CoordinateType<InnerContainerType<T>>& container, FunctionType f) { double
result{}; for (const auto& innerContainer : container) { for (auto i = std::begin(innerContainer); i !=
std::end(innerContainer) - 1; ++i) { for (auto j = i + 1; j != std::end(innerContainer); ++j) { result += f(i, j);
            }
        }
    }
    return result;
