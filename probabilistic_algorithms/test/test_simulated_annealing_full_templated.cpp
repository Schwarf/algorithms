...
{
    std::vector<std::vector<int>> locations = {{0, 0}, {1, 1}, {2, 0}, {3, 2},    {4, 1},
                                               {5, 3}, {6, 0}, {7, 2}, {27, -10}, {-6, -9}};
    ...
    while (tries)
    {
        double approximate_result = simulated_annealing_algorithm(
            locations, object_function, perturbation_function, initial_temperature, cooling_rate, minimal_temperature);
        tries--;
...
TEST(TestSimulatedAnnealing, simple_fully_templated3)
{
    ...
    int tries{10};
    int count_exact{};
    auto perturbation_function = TravelingSalesmanPerturbationFunction<double, std::vector, std::vector>();
    auto object_function = TravelingSalesmanObjectFunction<double, std::vector, std::vector>();
...
TEST(TestSimulatedAnnealing, difficult_fully_templated)
{
    ...
    auto sum_of_squared_difference =
        std::transform_reduce(approximate_results.begin(), approximate_results.end(), 0.0, std::plus<>(),
