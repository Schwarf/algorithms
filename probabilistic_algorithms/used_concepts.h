...
concept ReturnTypeIsDouble =
    std::same_as<std::invoke_result_t<FunctionType, InnerContainerType<T>, InnerContainerType<T>>, double>;


template <typename Functor, typename T, template <typename...> class ContainerType,
...
template <typename Functor, typename T, template <typename...> class ContainerType,
          template <typename...> class InnerContainerType>
concept PerturbationFunctor =
    requires(Functor functor, ContainerType<InnerContainerType<T>>& container, std::mt19937 random_engine) {
