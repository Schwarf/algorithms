...
// (possibly empty) inside one of these sentences such that the two sentences become equal. Note that the inserted
// sentence must be separated from existing words by spaces. For example, s1 = "Hello Jane" and s2 = "Hello my name is
// Jane" can be made equal by inserting "my name is" between "Hello" and "Jane" in s1. s1 = "Frog cool" and s2 = "Frogs
// are cool" are not similar, since although there is a sentence "s are" inserted into s1, it is not separated from
// "Frog" by a space. Given two sentences sentence1 and sentence2, return true if sentence1 and sentence2 are similar.
// Otherwise, return false. #GOOGLE
#include <sstream>
#include <string>
#include <vector>
