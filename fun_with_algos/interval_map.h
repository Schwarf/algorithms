...
public:
    ...
    std::map<KeyType, ValueType,
             std::conditional_t<std::is_floating_point_v<KeyType>, FloatComparator, std::less<KeyType>>>
        m_map;

    IntervalMap() { interval_map_[std::numeric_limits<KeyType>::min()] = ValueType{}; };

    ValueType const& operator[](const KeyType& key) const { return (--interval_map_.upper_bound(key))->second; }
    ...
    void add2(const KeyType& intervalBegin, const KeyType& intervalEnd, const ValueType& mappedValue)
    {
        ...
        {
            iteratorPositionIntervalEnd =
                interval_map_.insert(interval_map_.end(), std::make_pair(intervalEnd, (--lowerbound)->second));
        }
