...
public:
    ...
    //	(II)  Correctness: Your program should produce a working interval_map with the behavior described above. In
    //particular, pay attention to the validity of iterators. It is illegal to dereference end iterators. Consider using
    //a checking STL implementation such as the one shipped with Visual C++ or GCC. 	(III) Canonicity: The representation
    //in m_map must be canonical.
    //
    //	(IIII) Running time: Imagine your implementation is part of a library, so it should be big-O optimal. In
    //addition: 		                Do not make big-O more operations on K and V than necessary because you do not know how fast operations
    //on K/V are; remember that constructions, destructions and assignments are operations as well. 	                    Do not make more
    //than one operation of amortized O(log N), in contrast to O(1), running time, where N is the number of elements in
    //m_map.
    ...
    void assign(K const& keyBegin, K const& keyEnd, V const& val)
    {
        ...
        //		// [min, 1, 2] insert 5, lowerbound points to end -> [min, 1, 2, 5] and value of key 2 is now set to key
        //5
        //		// [min, 1, 10] insert 5, lowerbound points to 10 -> [min, 1, 5, 10] and value of key 10 is now set to
        //key 5 		if (iteratorPositionkeyEnd == m_map.end())
        //		{
        ...
        if (lowerbound == m_map.end() || keyEnd < lowerbound->first)
        { // Requirement (I) nothing else but the < operator is defined
