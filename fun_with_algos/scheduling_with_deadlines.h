...
// You are given a list of freelance jobs. Each job takes exactly 1 day to complete and can be done on any single day up
// to and including its deadline. You have only 7 days available (Day 1 through Day 7), and you can complete at most one
// job per day. Each job is represented as a map with two keys: "deadline": an integer deadline (may be greater than 7)
...
int optimal_freelancing(std::vector<std::unordered_map<std::string, int>> jobs)
{
    ...
    std::sort(jobs.begin(), jobs.end(),
              [](std::unordered_map<std::string, int>& m1, std::unordered_map<std::string, int>& m2)
