...
// LC-962
// A min-max-index-gap in an integer array nums is a pair (i, j) for which i < j and nums[i] <= nums[j]. The width of
// such a gap is j - i. Given an integer array input, return the maximum min-max-index-gap in input.
...
int largest_extremum_index_gap_sort(std::vector<T>& input, Comparator comparator = Comparator())
{
    ...
    std::sort(indices.begin(), indices.end(),
              [&](int i, int j) { return comparator(input[i], input[j]) || (input[i] == input[j] && i < j); });
