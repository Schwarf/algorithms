...
public:
    ...
    void setSelectClause(const std::string& select) { selectClause = select; }

    void setFromClause(const std::string& from) { fromClause = from; }

    void setWhereClause(const std::string& where) { whereClause = where; }

    void setOrderByClause(const std::string& orderBy) { orderByClause = orderBy; }
...
int main()
{
    SqlQuery query = SqlQueryBuilder().select("name, age").from("users").where("age >= 18").orderBy("name").build();
